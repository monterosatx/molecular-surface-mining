...
def output_pdb_as_xyzrn(pdbfilename, xyzrnfilename, keep_het=False):
    """
    ...
    for atom in struct.get_atoms():
        ...
        color = "Green"
        coords = None
        if (atomtype in radii and resname in polarHydrogens) or keep_het:
            if atomtype == "O":
                color = "Red"
            if atomtype == "N":
                color = "Blue"
            if atomtype == "H":
                if resname in polarHydrogens and name in polarHydrogens[resname]:
