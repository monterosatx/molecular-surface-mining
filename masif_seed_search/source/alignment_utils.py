...
def align_and_save(out_filename_base, source_structure):
...
def align_protein(name, \
    ...
    # mimicry paper modification: also filter if the desc_dist_score similarity is less than 25
    top_scorers = np.where((scores[:,0] > params['nn_score_cutoff']) & (scores[:,1] > 30))[0]
    ...
    if len(top_scorers) > 0:
        ...
        for j in top_scorers:
            ...
            if clashing_ca <= params['allowed_CA_clashes'] and clashing_total <= params['allowed_heavy_atom_clashes']:
                ...
                align_and_save(out_fn, source_structure_cp, )
                ...
                # Ignore surface alignment to make it faster, remove the following line. 
                # continue 
                print ('Surface alignment activated. This makes the code a lot slower. The surface alignment serves no purpose other than visualization.')
                
                # Align and save the ply file for convenience for full protein and patch.    
                mesh = Simple_mesh()
                ...
                # Save the matched patch.
                out_patch = Simple_mesh()
                out_vertices = np.asarray(all_source_patch[j].points)
                out_patch.vertices = out_vertices
                out_patch.set_attribute('vertex_x', out_vertices[:,0])
                out_patch.set_attribute('vertex_y', out_vertices[:,1])
                out_patch.set_attribute('vertex_z', out_vertices[:,2])
                nx = np.asarray(all_source_patch[j].normals)[:,0]
                ny = np.asarray(all_source_patch[j].normals)[:,1]
                nz = np.asarray(all_source_patch[j].normals)[:,2]

                out_patch.set_attribute('vertex_nx', nx)
                out_patch.set_attribute('vertex_ny', ny)
                out_patch.set_attribute('vertex_nz', nz)

                all_charges = mesh.get_attribute('vertex_charge')
                out_patch.set_attribute('vertex_charge', all_charges[all_source_idx[j]])

                # Get the faces for the patch.
                # Find the closest points to each point in the patch in the full mesh 
                kdt = cKDTree(mesh.vertices)
                dist, idx = kdt.query(out_patch.vertices)
                facestmp = []
                for f in mesh.faces:
                    if np.all(np.isin(f, idx)):
                        facestmp.append(f)
                facestmp = np.array(facestmp)

                # Change the index of the faces to match the new vertices.
                newidx = {idx[i]: i for i in range(len(idx))}
                newfaces = np.array([[newidx[x] for x in f] for f in facestmp])
                out_patch.faces = newfaces
                out_patch.save_mesh(out_fn+'_patch.ply')
                # 
